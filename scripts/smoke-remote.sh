...
# namespace, updates it, rolls it back, reads and writes its state, uploads,
# lists and deletes an asset, restricts it and checks the old address stops
# answering, then deletes it. The site is deleted on every exit path,
...
# that a signed-in stranger gets 404 from the restricted site's own host.
#
# Exit status is the number of failures, capped at 125.
set -u

BASE="${BASE:-}"
SKILL_VERSION="${SKILL_VERSION:-0.12.0}"
...
owner_host="$(host_of "$site_url")"
case "$owner_host" in
  *."$BASE") ok "site address is $site_url" ;;
...
echo "== the owner host $owner_host (wildcard DNS and certificate)"
expect 401 "hosted content needs a signed-in session" "$owner_host" "/$site/"
expect 404 "the owner host serves nothing at /admin" "$owner_host" /admin

echo "== state"
expect 200 "state read" "$owner_host" "/api/sites/$site/state/versioned" "${K[@]}"
ver="$(json 'd["version"]')"; ver="${ver:-0}"
expect 200 "state write" "$owner_host" "/api/sites/$site/state/versioned" -X PUT "${K[@]}" \
  -H "Content-Type: application/json" -d "{\"version\":$ver,\"state\":{\"smoke\":\"$site\"}}"
expect 200 "state read after write" "$owner_host" "/api/sites/$site/state/versioned" "${K[@]}"
if [ "$(json 'd["state"].get("smoke","")')" = "$site" ]; then ok "state round-trips"; else bad "state read did not return the write"; fi
expect 409 "a stale version is refused" "$owner_host" "/api/sites/$site/state/versioned" -X PUT "${K[@]}" \
  -H "Content-Type: application/json" -d "{\"version\":$ver,\"state\":{}}"

echo "== assets"
printf 'smoke asset\n' > "$work/notes.txt"
expect 2xx "asset upload" "$owner_host" "/api/sites/$site/assets" -X POST "${K[@]}" \
  -F "file=@$work/notes.txt;filename=notes.txt;type=text/plain"
asset_id="$(json 'd["id"]')"
if [ -n "$asset_id" ]; then
  ok "asset id returned"
  expect 200 "asset list" "$owner_host" "/api/sites/$site/assets" "${K[@]}"
  if grep -q '"notes.txt"' "$work/body"; then ok "asset appears in the list"; else bad "asset missing from the list"; fi
  expect 2xx "asset delete" "$owner_host" "/api/sites/$site/assets/$asset_id" -X DELETE "${K[@]}"
else
...
case "$restricted_host" in
  *--*."$BASE") ok "restricted site moved to $restricted_host" ;;
  *) bad "restricted site did not move to its own host (got '$restricted_host')" ;;
esac
expect 404 "the old address answers 404 to anyone" "$owner_host" "/$site/"
if [ -n "$restricted_host" ]; then
  expect 401 "the restricted host needs a signed-in session" "$restricted_host" /
  expect 401 "the restricted host refuses a wrong key" "$restricted_host" /api/site/state/versioned -H "X-API-Key: smoke-not-a-key"
