...
- A **network attacker**: can reach the base host and every owner host
  and site host over the public or corporate network, but holds no valid session, key,
  or platform credential. Such a person can read a site an admin has
  approved for the network, and nothing else.

**Trust boundaries.** Browser to site host; browser to owner host; browser
to base host; site host to its sibling site hosts and to its owner host;
owner and site hosts to base host (same process, different hostnames, and the
boundaries the whole subdomain-only design exists to hold —
`docs/site-isolation.md`); pod to database;
...
| S2 | Default admin key; admin was a synthetic principal with no row | Admin is an OIDC-identified person with a row; `ADMIN_API_KEY` removed entirely. The admin API accepts a browser session only, never an API key (`requireSessionAuth` in `internal/handler/admin.go`); the one exception is the offboard-scoped key on `POST /api/admin/users/disable`. The company-wide view of `GET /api/audit` and `GET /api/access` also needs an admin's browser session: an admin's key sees only their own namespace (`adminView` in `internal/handler/audit_access.go`), and `ADMIN_EMAILS` is re-applied to every account at startup (`db.SyncAdminEmails`, called from `cmd/server/main.go`), so removing someone from the list takes effect on the next restart without their signing in | Built |
...
| S7 | Hosted pages could share the control-plane origin until cutover | The cutover flag and the shared-origin path are deleted outright; only the subdomain shape exists. Since v1.3.0 every site, at every access level, is served on its own host (`<site>.<owner>.<base>`), so sibling sites of one owner are different origins — `internal/handler/host_gate.go`, `host.go`'s `Classify`/`SiteURL`. Until an owner's `*.<owner>.<base>` certificate is ready, that owner's sites are served at the fallback `<owner>.<base>/<site>/` and share the owner host's origin, as in v1.2 (only the owner's or team's own code runs there); see the accepted limitations. The site-facing API follows the same rule: a site's state and assets answer only on the host that serves the site, and on a site host the named shape is honoured only when it names that site (`serveSiteAPI` in `host_gate.go`). Old addresses redirect (301 GET/HEAD, 308 otherwise): owner-host paths to the site host once the certificate is ready, and v1.2 `<owner>--<site>.<base>` hosts to the site's current address. Every redirect is unauthenticated and computed from the address alone, with no lookup, so it confirms nothing about whether a site exists (`host_gate.go`'s `redirectToHost`). Pre-v1.3 team addresses redirect to their `team-<name>` equivalents only while no account holds the old name; a person who later signs in under that name takes the address | Built |
| S14 | Session cookies lacked `__Host-`; the admin cookie could not be revoked per browser | `__Host-` prefix on every session cookie (and on the search-session and per-site visit cookies); revocable session rows, one per sign-in. Every session cookie is bound to the host it was minted for and accepted only there: a base-host cookie carries no host and a hand-off cookie names the owner host or site host it was minted for (`internal/auth/session_cookie.go`'s `SignHostSession`). Hosted content checks it in `hostsession.go`'s `VerifyHostedSession`; `auth.Middleware` refuses a host-bound cookie on the base host and, for the site-facing API, requires the cookie's host to equal the requested host (`auth.WithExpectedSessionHost`, set by `host_gate.go`'s `serveSiteAPI`) on top of the session row's validity; the base-host pages that read the cookie themselves (`dashboard.go`, `admin.go`, `showcase.go`) use `auth.VerifyBaseSessionCookie`. Hand-off codes are stored as SHA-256 only and spent by the first redemption attempt whatever its outcome (`internal/db/handoff.go`), so a code delivered to a browser that cannot redeem it (the popup chain a page on its own owner host could otherwise run) cannot be carried off and redeemed with the attacker's nonce; service-worker script fetches are refused on hosted hosts so a site's root-scoped worker cannot intercept `/auth/session` | Built |
...
| S8 | No `Referrer-Policy`, no `Cache-Control`, `frame-ancestors` withheld on hosted content | `Referrer-Policy: strict-origin-when-cross-origin` on every host (`internal/handler/security.go`'s `SecurityHeaders`, unconditional as of the second review round); `Cache-Control: private, no-cache` on every owner-host and site-host response and `Cache-Control: no-store` on an authenticated base-host request; `Cross-Origin-Resource-Policy: same-origin` on every owner-host and site-host response — `host_gate.go`'s `applyOwnerHostSecurity`; a `Sec-Fetch-Site`/`Sec-Fetch-Dest` refusal on top for browsers that ignore CORP; framing stays an owner choice | Built |
...
| S18 | Extension denylist rather than allowlist for uploads | Kept deliberately for a site's own archive contents: uploads are served as bytes, never executed. The denylist covers source scripts plus Windows and other executables, installers, packages and disk images (`.exe .dll .msi .hta .vbs .jar .apk .iso` and the rest in `docs/configuration.md`), so a company hostname does not hand out installers; `CLAMD_ADDR` is the scan for the bytes. A top-level `_assets/` archive entry is refused at extraction (`internal/tarball/extract.go`'s `isReservedAssetsPath`) so an upload cannot shadow the asset route. Uploaded assets themselves (`POST /api/sites/{site}/assets`) are a closed content-type allowlist over the sniffed bytes, never the declared type or extension (`internal/storage/assets.go`'s `classifyContentType`) | Built |
...
|---|---|---|
| Owner-hosts reconciler credential | The server's pods still mount no Kubernetes credential (`automountServiceAccountToken: false`). Issuing each owner's `*.<owner>.<base>` certificate runs in a separate Deployment, `simple-host-owner-hosts` (`internal/ownerhosts`, `deploy/components/owner-hosts`), whose ServiceAccount is the one token in the install. Its Role is namespaced: `ingresses` get/list/create/patch/delete, `certificates.cert-manager.io` get, no Secrets (readiness comes from the Certificate's status, never the key). It connects to Postgres as the application role and writes only `owner_hosts`. Threat: whoever holds that token can create Ingresses in the namespace, routing any hostname to a Service there; it cannot read Secrets or act outside the namespace. Protect it like the namespace's other credentials; with `OWNER_CERTS=manual` the component and the token are left out entirely | Built |
| Cookie tossing between site hosts | A page on `todo.alice.<base>` can ask the browser to set a cookie with `Domain=alice.<base>`, which the browser would then send to `alice.<base>` and every sibling `*.alice.<base>`. Every cookie the server reads is named `__Host-` (`__Host-sh_session`, `__Host-sh_handoff`, `__Host-sh_oauth`, the search-session and visit cookies). A browser stores a `__Host-` cookie only if it is `Secure`, `Path=/` and has no `Domain` attribute, so it is host-only: a sibling cannot set one that reaches another host, and cannot overwrite the owner host's or a sibling's. A tossed cookie with any other name is never read. The session value is also signed and bound to the host it was minted for (next row) | Built |
| Host-bound session cookie | A hosted-content (owner-host or site-host) session cookie's signed payload now carries the host it was minted for (`internal/auth/session_cookie.go`'s `SignHostSession`); `VerifyHostedSession` refuses a payload whose host claim does not match the request's own classified host, and refuses a payload with no host claim at all outright. This closes the gap `test/pentest`'s `TestSessionFixationAcrossHosts` demonstrated directly — a captured cookie value, presented by a non-browser client, would otherwise authenticate on every host the same person had ever opened, since the underlying session row is shared by design. | Built |
| Anonymous network access | A `network` site is open without sign-in only after an admin approves the owner's request (`/admin` "Access requests", `POST /api/admin/access-requests/{owner}/{site}/approve`); until then it keeps its level. The person who made the request can never approve it, and with `NETWORK_ACCESS_APPROVALS=2` two different admins must (`network_access_approvals`, counted under a lock on the site row, so concurrent approvals cannot open it twice or on one approval). With no valid host session, the host gate lets through only that site's pages and assets on the host that serves it and a `GET` of its saved data (`host_gate.go`'s `requireHostSessionOrNetwork`, and the anonymous branch of `serveSiteAPI`). It cannot write saved data, upload or delete assets, reach the management API, or open any other site. Every other hosted-host protection (`nosniff`, CORP, no service workers) stays. The owner can drop the level at any time; an admin can revoke to `company`. Each step is audited | Built |
| Scoped API keys | An API key carries a scope fixed at mint (`api_keys.scope`, 0035): `publish` (the default: deploy, update, roll back and list sites and versions, saved data, assets, `/api/me`, `/mcp`), `full` (everything the person can do except administration), or `offboard` (admins only; `POST /api/admin/users/disable` and nothing else). `auth.Middleware` checks it against the matched route, deny by default, on every request including the ones `/mcp` makes in process; a test fails when a registered route is left unclassified. Keys minted before v1.2 became `full`. A leaked CI key no longer reaches site deletion, access levels, viewers, teams, logs or admin routes | Built |
| Connector tokens only on `/mcp` | An OAuth access token (`Authorization: Bearer`) is accepted only on `/mcp` and on the in-process requests the MCP server makes for it (a context mark set by `ProtectMCP`, which no client can send); a direct REST call with one is refused. A token copied out of an AI app is limited to what the MCP tools expose | Built |
| Bounded lifetimes and offboarding | `SESSION_TTL`/`SESSION_IDLE` (defaults `8h`/`30m`, capped at `24h`/`8h`), `OAUTH_ACCESS_TTL`/`OAUTH_REFRESH_TTL` (defaults `1h`/`30d`, capped at `24h`/`90d`, the refresh lifetime counted from the connecting sign-in, never extended by rotation) and `API_KEY_MAX_DAYS` (at most 365) are refused at startup when out of range, so a typo cannot make a credential permanent. Disabling a person (the `/admin` page, or an admin's `offboard` key calling `POST /api/admin/users/disable` by email for HR automation) ends their sessions, keys and connected apps at once. INSTALL.md, "Sessions and leavers": disable in the IdP **and** in Simple Host | Built |
...
audit trigger, the hand-off code spent on any attempt) and v1.3.0's
per-site hosts (the site-host API rule, the owner-host and v1.2 redirects)
are covered by unit tests that `make test` runs; the pen-test suite has not
been re-run as a whole against them. v1.3.0's fallback, certificate issue,
switch to site hosts and redirects were checked end to end on kind with
ingress-nginx and cert-manager (CHANGELOG, v1.3.0).
...
| Sibling-origin subresource loads of a site host | `<script src>`, `<link>`, `<img>`, `fetch` with `credentials: include` from another owner or site host, with and without `Sec-Fetch-*` headers | scripted: `test/pentest/sibling_origin_test.go` (the "with" cases); manual: a real browser's `<script src>`/`<img>` fetch against a site host is not exercised by a Go HTTP client and needs a hands-on check | Pass (scripted half), recorded against the pre-v1.3 shape (a restricted site's `<owner>--<site>.<base>` host, from an owner host); not re-run against v1.3 site hosts. Manual: not run — see "Browser-only checks" below |
...
| Owner-host and v1.2 redirects as an open redirect | Put foreign hosts, `//` shapes and CRLF in the path, query and `Host` of `<owner>.<base>/<site>/...`, `/api/sites/<site>/...` and `<owner>--<site>.<base>`; the `Location` must always be one site label under the owner's host | scripted: `test/pentest/owner_host_redirect_test.go` (added in v1.3.0) | Not part of the recorded run above |
...
**Browser-only checks (not run).** With two sites of the same owner,
`secret.alice.<base>` (access level `specific`) and `todo.alice.<base>`,
signed in as a named viewer of both, from a page on `todo.alice.<base>`:

1. `fetch('https://secret.alice.<base>/api/site/state', {credentials: 'include'})`
   must fail (no readable response), as must `<script src>`, `<link>` and
   `<img>` pointing at the other site's host.
2. `fetch('/api/sites/secret/state')` on `todo.alice.<base>` must return 404.
3. In the browser's developer tools, the `__Host-` session cookie set for
   `todo.alice.<base>` must not be sent to `secret.alice.<base>`,
   `alice.<base>` or the base
...
- **Same-owner sites share an origin only until the owner's certificate is
  ready.** Once an owner's `*.<owner>.<base>` certificate is ready, every
  site of theirs has its own origin: no shared cookies, storage or DOM, and
  a page on one site's host cannot name a sibling in its state or asset
  calls. Before that (a new owner's first site, for as long as issuance
  takes; indefinitely with `OWNER_CERTS=auto` and no reconciler deployed),
  the owner's sites are served at `<owner>.<base>/<site>/` and share the
  owner host's origin, as in v1.2. The only code on that host is the
  owner's (or the team's) own. There, a page can name a sibling site in
  its state or asset calls, and the audit record distinguishes the claim
  (`via_site`) from what the browser can corroborate
  (`via_site_observed`). See `docs/site-isolation.md`.
...
- **A few audit writes are still best-effort.** Everything else commits
  ...
  - `access_denied` (`host_gate.go`, `admin.go`), `sign_in_failed`
    (`auth.go`), `upload_infected` (`upload_limits.go`) and `admin_export`
    (`admin_export.go`): refusals and a read, with no write to tie the row
    to.
