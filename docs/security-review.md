...
| S6 | No private sites; serving never consulted a visibility flag | Every view requires a signed-in host session; a site may list named viewers, enforced by `viewerAllowed` — `internal/db/site_viewers.go`'s `ViewerAllowed`, `internal/handler/host_gate.go`'s `checkSiteAccess`/`checkViewerAllowed` | Built |
...
| S9b | No access log; who viewed a site was never recorded | `access_log` and its batching `AccessWriter` (`internal/audit/access_writer.go`) are wired into `serveSite`'s status recorder (`serve.go`), enqueuing one row per hosted-content response including the owner's and editors' own | Built |

### (d) Hardening beyond the audit

Two controls added on top of the 21 findings above.
...
- **There is no site-transfer feature and no per-site write-mode
  setting**, so there is nothing to audit for either. The schema's
  `state_write_mode` column exists but nothing sets it: every site's
  state is writable by anyone who may view it.
