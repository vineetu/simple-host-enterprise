...
func TestHTTPSRedirectIsHostAware(t *testing.T) {
	...
	cases := []struct {
		...
		{name: "site host", host: "a.b.safe.example", want: "https://a.b.safe.example:8443" + path},
		{name: "legacy site host", host: "b--a.safe.example", want: "https://b--a.safe.example:8443" + path},
		{name: "three labels deep", host: "a.b.c.safe.example", want: "https://safe.example:8443" + path},
		{name: "punycode site host", host: "xn--a.b.safe.example", want: "https://safe.example:8443" + path},
...
func TestPublicSearchHandlerConstructionAndRouteRegistration(t *testing.T) {
	...
	// the session requirement around search, so a pass-through stands in
	// for the real auth middleware.
