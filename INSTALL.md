...
Managed Postgres 16, with TLS enforced and point-in-time recovery on. The
per-cloud specifics are in `docs/cloud/aws.md`, `docs/cloud/gke.md`,
`docs/cloud/aks.md`, and `docs/cloud/oci.md`, section 2 of each.
...
Read `docs/cloud/aks.md` section 0 before you try.
...
- Azure: Blob Storage has no S3 API. Read `docs/cloud/aks.md` section 3 and
  pick MinIO (`deploy/components/minio`) or an S3-to-Blob proxy with the
  human before going further.
...
| `BACKUP_STORAGE_ENDPOINT`, `BACKUP_STORAGE_REGION`, `BACKUP_STORAGE_BUCKET`, `BACKUP_STORAGE_PREFIX` | From section 3 and `docs/cloud/*.md` |
...
- **Okta.** Applications → Create App Integration → OIDC - OpenID Connect →
  ...
  If token exchange fails with `invalid_client`, the app's client
  authentication must accept the client secret sent in the request body.
...
- **Microsoft Entra ID.** App registrations → New registration, single
  tenant. Redirect URI: platform "Web". Certificates & secrets → New client
  secret; copy the **Value**, not the Secret ID, and note its expiry.
  Issuer: `https://login.microsoftonline.com/<tenant-id>/v2.0`. Entra only
...
- **Any other OIDC provider.** Same generic settings. Every OIDC call the
  ...
  provider, so the issuer must be reachable from inside the cluster.
...
an IP means an `A` record. Per-cloud notes: section 5 of each
`docs/cloud/*.md`.
...
can solve DNS-01 for `<base>` and its name. Worked examples: section 6 of
`docs/cloud/aws.md` (Route 53), `gke.md` (Cloud DNS), `aks.md` (Azure DNS),
`oci.md` (no built-in solver; three options). Scope its DNS credential to
the one zone.
...
| Sign-in fails at token exchange | Wrong client secret, or the provider does not accept the secret in the request body. Re-enter with section 4's hidden prompt, re-apply, restart the Deployment. |
...
- **Upgrade**: `docs/install.md` section 10. Resolve the new image's digest
